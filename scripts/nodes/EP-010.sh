...
case "$mode" in
  ...
  M4) python3 scripts/node-artifact-check.py EP-010 M4; cargo test --locked -p nexus-connectors ep010_failure; sh scripts/security-check.sh; sh scripts/license-gate.sh ;;
  M5|verify)
