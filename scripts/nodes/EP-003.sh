...
case "$mode" in
  ...
  M4) python3 scripts/node-artifact-check.py EP-003 M4 && cargo test --locked -p nexus-identity ep003_failure && uv run --frozen pytest tests/identity -q -o python_functions="ep003_failure_*" && sh scripts/security-check.sh && sh scripts/license-gate.sh || rc=$? ;;
  M5|verify)
      python3 scripts/node-artifact-check.py EP-003 M5 \
      && cargo test --locked -p nexus-identity \
      && cargo test --locked -p nexus-presence \
      && uv run --frozen pytest tests/identity -q -o python_functions="ep003_failure_*" \
