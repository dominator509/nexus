...
mode="${1:-verify}"
case "$mode" in
  M1) python3 scripts/node-artifact-check.py EP-021 M1 ;;
  M2) python3 scripts/node-artifact-check.py EP-021 M2; uv run --frozen pytest tests/voice/core -q -k ep021_unit ;;
