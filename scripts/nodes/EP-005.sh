...
case "$mode" in
  ...
  M4) python3 scripts/node-artifact-check.py EP-005 M4 && cargo test --locked -p nexus-events ep005_failure && cargo test --locked -p nexus-nats ep005_failure && sh scripts/security-check.sh && sh scripts/license-gate.sh || rc=$? ;;
  M5|verify)
