...
case "$mode" in
  ...
  M3) python3 scripts/node-artifact-check.py EP-007 M3 && uv run --frozen pytest tests/auth -q --tb=native -o python_functions="ep007_integration_*" || rc=$? ;;
  M4) python3 scripts/node-artifact-check.py EP-007 M4 && cargo test --locked -p nexus-auth ep007_failure && uv run --frozen pytest tests/auth -q --tb=native -o python_functions="ep007_failure_*" && sh scripts/security-check.sh && sh scripts/license-gate.sh || rc=$? ;;
  M5|verify)
      python3 scripts/node-artifact-check.py EP-007 M5 \
      && cargo test --locked -p nexus-auth \
      && uv run --frozen pytest tests/auth -q --tb=native -o python_functions="ep007_failure_*" \
