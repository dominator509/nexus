#!/usr/bin/env sh
# EP-029 M1 gate: run the nexus-social contract suite through the REAL
# cargo test machinery with vacuity guards.
#
# The M1 changed-file fence is crates/nexus-social/ (contract crate)
# plus the node script and plan files, so the authoritative gate is the
# nexus-social cargo suite (unit + dependency-direction) plus fmt/clippy
...
# dependency-direction test, an EP-029-owned sentinel test name, and
# zero ignored/filtered tests.
set -eu
export CI=true
export CARGO_TERM_COLOR=never

log="/tmp/ep029-m1-tests.log"
: > "$log"

fail() {
  echo "EP-029 M1 gate: FAIL - $1" >&2
  tail -40 "${2:-/dev/null}" >&2 2>/dev/null || true
  exit 1
}
ok() { echo "EP-029 M1 gate: $1"; }

# Vacuity guard 0: the contract crate must exist.
if [ ! -f crates/nexus-social/Cargo.toml ]; then
  fail "crates/nexus-social/Cargo.toml missing"
fi

# Vacuity guard 0b: the owned production sources must exist.
for f in src/lib.rs src/error.rs src/vocabulary.rs src/model.rs \
         src/capability.rs src/policy.rs src/provider.rs \
         tests/dependency_direction.rs; do
  if [ ! -f "crates/nexus-social/$f" ]; then
    fail "crates/nexus-social/$f missing"
  fi
done
...
if ! "${CARGO_BIN:-$HOME/.cargo/bin/cargo}" test --locked -p nexus-social --all-targets >>"$log" 2>&1; then
  fail "cargo test -p nexus-social --all-targets failed" "$log"
fi
...
# Vacuity guard 4 (anti-masking): an EP-029-owned sentinel test must be
# observed. This fails if the gate accidentally executes only a prior
# node's tests.
if ! grep -q 'ep029_unit_variants_preserve_single_objective .* ok' "$log"; then
...
for f in .agent/milestone-files/EP-029-M1.txt .agent/node-contracts/EP-029.md \
