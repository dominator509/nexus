...
/**
 * Apply the one-package bundle's own DDL to the clean target. The
 * bundle carries the migration SQL; the deployment consumes the
 * package's copy, not the dev tree (proof the package performs its own
 * prerequisites).
 */
export function applyBundleMigrations(
  stack: TestStack,
  bundleDir: string,
): void {
  const sql = join(bundleDir, "migrations", "001_onboarding.sql");
  const res = spawnSync(
    "bash",
    [
      "-c",
      `PGPASSWORD=${PG_PASSWORD} psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -p ${stack.pgPort} -U ${PG_USER} -d ${PG_DB} -f '${sql}'`,
    ],
    { encoding: "utf8" },
  );
  if (res.status !== 0) {
    throw new Error(
      `apply bundle migrations failed: ${res.stderr || res.stdout}`,
    );
  }
}
...
/**
 * Probe the real runtime's durable boundary: the onboarding tables must
 * exist and carry exactly the rows the journey wrote. This is the
 * canonical application health gate (SELECT against the live schema),
 * distinct from process/container start.
 */
export function runtimeHealth(stack: TestStack): RuntimeHealth {
  const res = spawnSync(
    "bash",
    [
      "-c",
      `PGPASSWORD=${PG_PASSWORD} psql -h 127.0.0.1 -p ${stack.pgPort} -U ${PG_USER} -d ${PG_DB} -Atc "SELECT to_regclass('onboarding_owner'), to_regclass('onboarding_deployment_intent'), (SELECT count(*) FROM onboarding_owner), (SELECT count(*) FROM onboarding_deployment_intent), (SELECT count(*) FROM onboarding_integration_state)"`,
    ],
    { encoding: "utf8" },
  );
  if (res.status !== 0) {
    throw new Error(`runtime health probe failed: ${res.stderr || res.stdout}`);
  }
  const parts = res.stdout.trim().split("|");
  return {
    owner_table: parts[0] === "onboarding_owner",
    intent_table: parts[1] === "onboarding_deployment_intent",
    owner_rows: Number(parts[2]),
    intent_rows: Number(parts[3]),
    integration_rows: Number(parts[4]),
  };
}
