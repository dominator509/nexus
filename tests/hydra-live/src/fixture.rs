...
//! over real std::net sockets (the versioned canonical surface from
//! schemas/hydra/). Mocks control the PEER only; the transport/adapter
...
/// dispatches each request to the handler (supports context read then
/// action submit then readback in one proof).
...
/// Server that accepts one connection, keeps the socket OPEN, and never
/// completes the HTTP response (silent peer).
pub fn spawn_silent_server() -> (u16, JoinHandle<()>) {
    let listener = TcpListener::bind("127.0.0.1:0").expect("bind");
    let port = listener.local_addr().expect("addr").port();
    let handle = thread::spawn(move || {
