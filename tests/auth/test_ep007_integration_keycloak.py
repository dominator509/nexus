...
def _wait_for_keycloak(container: str, timeout: float = 120.0) -> int:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            time.sleep(1.0)
    raise TimeoutError(
        f"Keycloak discovery {discovery} not ready within {timeout}s"
    ) from last_error
...
def _start_keycloak() -> tuple[str, int, str, str]:
    ...
    subprocess.run(
        [
            ...
            "-e",
            f"KC_BOOTSTRAP_ADMIN_PASSWORD={admin_pw}",
            "-v",
            f"{REALM_JSON}:/opt/keycloak/data/import/nexus-realm.json:ro",
...
def _admin(port: int, token: str, method: str, path: str, body=None):
    ...
    try:
        ...
        raise AssertionError(f"admin {method} {path}: HTTP {exc.code}: {detail}") from None
...
def _set_default_scopes(port: int, token: str, client: dict, wanted: set[str]) -> None:
    ...
    """
    scopes = {
        s["name"]: s["id"] for s in _admin(port, token, "GET", "/admin/realms/nexus/client-scopes")
    }
    current = (
        _admin(
            port, token, "GET", f"/admin/realms/nexus/clients/{client['id']}/default-client-scopes"
        )
        or []
    )
...
def _bootstrap(port: int, admin_user: str, admin_pw: str, owner_temporary: bool = False) -> dict:
    ...
    if not users_created_inline:
        _admin(
            ...
            token,
            "PUT",
            f"/admin/realms/nexus/users/{owner_id}",
            {
                "id": owner_id,
                "username": "owner",
                "enabled": True,
        ...
        _admin(
            ...
            token,
            "PUT",
            f"/admin/realms/nexus/users/{owner_id}/reset-password",
            {
                "type": "password",
                "value": owner_password,
                "temporary": owner_temporary,
    ...
    if "nexus" not in scopes:
        _admin(
            ...
            token,
            "POST",
            "/admin/realms/nexus/client-scopes",
            {
                "name": "nexus",
                "protocol": "openid-connect",
        ...
        scopes = {
            s["name"]: s for s in _admin(port, token, "GET", "/admin/realms/nexus/client-scopes")
        }

    # --- nexus.read client scope (canonical protected-resource scope) -----
    # Mirrors the "nexus" tenant-mapper scope pattern: the realm fixture
    # never declares a clientScopes section (a Keycloak 26.7.0 import
    # quirk silently drops ALL built-in client scopes when it does), so
    # custom scopes are provisioned here through the real Admin API.
    if "nexus.read" not in scopes:
        _admin(
            port,
            token,
            "POST",
            "/admin/realms/nexus/client-scopes",
            {
                ...
                "protocolMappers": [
                    {
                        "name": "nexus-read-scope-mapper",
                        "protocol": "openid-connect",
                        "protocolMapper": "oidc-hardcoded-claim-mapper",
                        "consentRequired": False,
                        "config": {
                            "claim.name": "scope",
                            "claim.value": "nexus.read",
                            "jsonType.label": "String",
                            "access.token.claim": "true",
                            "id.token.claim": "false",
                            "userinfo.token.claim": "false",
    ...
    _set_default_scopes(
        port, token, app, {"basic", "profile", "email", "roles", "acr", "nexus", "nexus.read"}
    )
    _set_default_scopes(port, token, scheduler, {"basic", "roles", "nexus"})
    _set_default_scopes(port, token, connector, {"basic", "nexus"})

    scheduler_secret = _admin(
        port, token, "POST", f"/admin/realms/nexus/clients/{scheduler['id']}/client-secret"
    )["value"]
    connector_secret = _admin(
        port, token, "POST", f"/admin/realms/nexus/clients/{connector['id']}/client-secret"
    )["value"]

    # --- least-privilege role mapping -------------------------------------
    scheduler_service = _admin(
        port,
        token,
        "GET",
        "/admin/realms/nexus/users?username=service-account-nexus-scheduler&exact=true",
    )[0]
    connector_service = _admin(
        port,
        token,
        "GET",
        "/admin/realms/nexus/users?username=service-account-nexus-connector-runtime&exact=true",
    )[0]
    _admin(
        port,
        token,
...
def _verify_rs256(token: str, jwks: list[dict]) -> bool:
    ...
    digest_info = (
        b"\x30\x31\x30\x0d\x06\x09\x60\x86\x48\x01\x65\x03\x04\x02\x01\x05\x00\x04\x20" + digest
    )
...
class BoundaryValidator:
    ...
    def rejections(self, claims: dict) -> list[str]:
        ...
        aud = claims.get("aud")
        aud_ok = self.audience in aud if isinstance(aud, list) else aud == self.audience
        if not aud_ok:
...
def _complete_auth_code_pkce_flow(
    ...
    *,
    ...
    try:
        ...
        post_login = _submit_form(
            opener, action, fields, {"username": username, "password": password}, auth_url
        )
        _strip_secure_flags(jar)
        ...
        if server.captured is None:
            raise AssertionError(
                "authorization code was not delivered to the registered redirect URI"
            )
...
def ep007_integration_owner_account_required_action_diagnosis() -> None:
    ...
    with _keycloak() as (_name, port, au, ap):
        ...
        owner = _admin(port, token, "GET", "/admin/realms/nexus/users?username=owner&exact=true")[0]
        ...
        creds = _admin(port, token, "GET", f"/admin/realms/nexus/users/{owner['id']}/credentials")
        ...
        for c in pw_creds:
            ...
            assert cd.get("temporary") in (None, False), (
                "standard bootstrap credential is not temporary"
            )
        ...
        required_attrs = {a["name"] for a in profile.get("attributes", []) if a.get("required")}
...
def ep007_integration_authorization_code_pkce_owner_login() -> None:
    ...
    with _keycloak() as (_name, port, au, ap):
        ...
        validator = BoundaryValidator(
            _issuer(port), HUMAN_CLIENT, required_scopes=("openid", "profile")
        )
...
def ep007_integration_client_credentials_service_identity() -> None:
    ...
    with _keycloak() as (_name, port, au, ap):
        ...
        assert claims["sub"] != owner_claims["sub"], (
            "token must be a service account, not the owner"
        )
    ...
    with _keycloak() as (_name, port, au, ap):
        ...
        try:
            _complete_auth_code_pkce_flow(
                port,
                username="owner",
                password=boot["owner_password"],
                verifier=verifier_a,
                expect_state="attacker-controlled-state",
            )
        ...
        nonce_validator = BoundaryValidator(
            _issuer(port), HUMAN_CLIENT, expected_nonce="attacker-nonce"
        )
        ...
        issuer_validator = BoundaryValidator(
        ...
        audience_validator = BoundaryValidator(
            _issuer(port), "evil-client", required_scopes=("openid", "profile")
        )
