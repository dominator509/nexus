//! EP-016 dependency-direction guard for `nexus-memory-workers`
//! (SPEC-001): the memory workers crate must not drag infrastructure,
//! network, HTTP, vendor, or later-node crates into its production
//! tree. All I/O (retrieval, sources, graph, model, clock) is injected
//! through ports.
...
fn ep016_unit_dependency_direction() {
    ...
    assert!(
        violations.is_empty(),
        "nexus-memory-workers production tree violates dependency direction:\n{}",
        violations.join("\n")
    );
}
