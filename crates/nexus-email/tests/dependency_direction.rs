    ...
    for line in dep_section.lines() {
        ...
        assert!(
            name.starts_with("nexus-domain")
                ...
                // sha256 digest for Attachment artifact evidence
                // (SPEC-014; nexus-telephony precedent). MIT/Apache-2.0,
                // audit-gated.
                || name.starts_with("sha2"),
            "unexpected dependency {name:?} in nexus-email"
        );
    }
}
