...
- [x] M2: Core behavior and deterministic invariants (2026-08-19; commit 5b97a26)
- [x] M3: Real dependency and transport integration (2026-08-19; commit 110a60a)
- [x] M4: Forced failures, abuse cases, and observability (2026-08-19; commit 67d0e2d)
- [x] M5: Live-fire, operations, and node closure (2026-08-19; gate + node sentinels observed; commit pending)
...
- 2026-08-19 M3: the pre-created node M3 branch ran `python3 scripts/node-artifact-check.py EP-028 M3; cargo test --locked -p nexus-hydra ep028_integration` - artifact check plus a cargo filter against the CONTRACT crate where no `ep028_integration_*` tests exist (zero-match filter exits 0 - the EP-001 masking class, and the wrong crate entirely). Replaced with `sh scripts/ep028-m3-tests.sh` (real connector suite + schema parity + M1/M2 regressions + anti-masking sentinel).
- 2026-08-19 M3: there is NO Hydra component in COMPONENT_REGISTRY (Postiz is EP-029's node); per anti-hallucination the transport integration uses the versioned canonical surface from schemas/hydra/ and proves contract behavior over REAL std::net sockets against a controlled local HTTP fixture emitting REAL Hydra-shaped responses (EP-026 M3 Microsoft Graph precedent). Real provider certification is DEFERRED and recorded.
- 2026-08-19 M3: two real-socket test defects found and fixed: (1) the silent-peer fixture closed the socket after one read, so reqwest surfaced ExternalProvider instead of Timeout - the fixture now keeps the socket OPEN past the transport timeout; (2) submit-then-readback needs a server that accepts MULTIPLE sequential connections (spawn_multi_server), because a one-request server makes the readback hit a refused port.
- 2026-08-19 M4: the intended `ep028_failure_unknown_provider_vocabulary_fails_closed` test exposed a REAL production defect, not a test bug: `HttpHydraTransport::submit_action`/`read_action` returned the provider state string UNVALIDATED, so a fabricated state (`FABRICATED_STATE`) surfaced as `Ok` at the transport boundary. The adapter failed closed later (its own parse), but the transport is the vocabulary boundary - it must fail closed independently (directive M: fail at BOTH layers). Fixed in `connectors/hydra/src/transport.rs`: canonical `HydraActionState` parse before returning; unknown state -> HydraErrorCode::ExternalProvider. This is the M2 transport's latent gap, exposed and owned by M4's failure suite (same class as the EP-027 evidence-path defect).
- 2026-08-19 M4: `tests/hydra/ops/hydra-diag.sh` had a fail-closed defect: `curl ... | head -c 400` makes the pipeline exit status head's (always 0), so a REFUSED endpoint reported `reachable=yes` rc=0 (observed: `pipe_rc=0` while `curl_rc=7`). Fixed by capturing curl's output first (`out="$(curl ...)" || return 1`), then truncating with head; unreachable now exits 3 fail-closed. This is exactly the directive P hazard: a diagnostic must never print "healthy" merely because the script exists.
- 2026-08-19 M4: the M4 gate itself had a `set -e` defect: `sh tests/hydra/ops/hydra-diag.sh diagnose http://127.0.0.1:1; diag_rc=$?` - the non-zero exit (3) aborted the gate BEFORE `diag_rc=$?` ran, so the fail-closed assertion never executed and the gate died silently at rc=3. Fixed with `diag_rc=0; sh ... || diag_rc=$?` so the intended assertion actually runs.
- 2026-08-19 M4: compile-boundary discoveries were REAL Rust ownership facts, not test-workarounds: `ApprovalClass`/`BusinessId`/`PersonId`/`TenantId` are owned by `nexus-domain` (typed ids + canonical vocabulary), NOT re-exported by `nexus-hydra`; and `read_context`/`submit_action` are `HydraProvider` trait methods, so the trait must be imported for the adapter's inherent impl to be callable (E0599). No re-exports were added to nexus-hydra and no inherent wrappers were added to HydraAdapter - the package boundary (domain ids -> nexus-domain; Hydra vocabulary/contracts -> nexus-hydra) is preserved.
- 2026-08-19 M5: the pre-created node M5|verify branch ran `python3 scripts/node-artifact-check.py EP-028 M5; cargo test --locked -p nexus-hydra; sh scripts/live-fire/LF-015.sh; sh scripts/live-fire/LF-025.sh` - artifact check plus the M1 CONTRACT crate suite plus proof-runner PLACEHOLDERS for both live-fire scripts (proof-runner.sh certifies nothing; EP-001 masking class). Replaced with `sh scripts/ep028-m5-tests.sh` (real live-fire suite + current-run evidence + redaction + regressions + anti-masking sentinels).
- 2026-08-19 M5: evidence-path discovery for a NESTED test crate: `tests/hydra-live/` is two levels below the workspace root, so `CARGO_MANIFEST_DIR.parent()` resolves to `tests/`, not the root (evidence was written to `tests/.agent/state/evidence/`). Fixed with a workspace-root WALK that ascends until the directory whose Cargo.toml contains `[workspace]`; evidence now lands at `.agent/state/evidence/` regardless of crate nesting.
- 2026-08-19 M5: the M5 gate vacuity guard 1 pattern `running [1-9][0-9]* tests` (plural) does not match cargo's singular `running 1 test` line; fixed to `running [1-9][0-9]* test` (matches both singular and plural).
- 2026-08-19 M5: prettier flagged the ops runbook (markdown style) and the blueprint validator flagged non-ASCII em-dashes in docs/operations/EP-028-hydra.md; normalized to ASCII and prettier --write; format gate green.
...
- 2026-08-19 M3 | Canonical JSON Schemas under schemas/hydra/ as the cross-language contract source (business-context, context-projection, capability-map, action-request, event-envelope; JSON Schema 2020-12, `additionalProperties: false`, vocabulary-locked enums). Real Rust serialization is validated against the REAL schemas with the REAL jsonschema validator (EP-010/EP-018 pattern), so serde/schema drift fails the build. Evidence: `ep028_integration_*_matches_canonical_schema` tests green. Alternatives: hand-written duplicate DTOs (rejected: schemas/README forbids), no schema parity (rejected: cross-language contract drift).
- 2026-08-19 M3 | Real transport integration over REAL sockets: the production `HttpHydraTransport` is exercised against a controlled local HTTP fixture emitting REAL Hydra-shaped responses (200 context/capabilities/action envelope, 401/403/404/409/429/500/503, malformed JSON, silent peer, refused port). Mocks control the peer only; the transport is never mocked (EP-026 M3 directive L pattern). Evidence: `ep028_integration_read_context_real_http`, `ep028_integration_status_classification_real_http`, `ep028_integration_silent_peer_times_out_real_http`, `ep028_integration_refused_port_unavailable_real_http` green. Certification boundary: TRANSPORT/PROTOCOL_CERTIFIED for the canonical surface; real Hydra provider NOT ASSERTED (no component selected; deferred).
- 2026-08-19 M4 | Fail-closed unknown/fabricated state at BOTH layers (directive M): the transport is the vocabulary boundary, so `HttpHydraTransport::submit_action`/`read_action` now parse the returned state against the canonical `HydraActionState` ladder BEFORE returning; unknown state -> `ExternalProvider`. The adapter keeps its own parse as defense-in-depth. Evidence: `ep028_failure_unknown_provider_vocabulary_fails_closed` (transport) + `ep028_failure_unknown_action_state_fails_closed_through_adapter` (adapter) green. This closes the M2 transport gap (previously returned raw provider strings; the M3 submit+readback used canonical SUBMITTED so it could not catch the gap). Alternatives: leave validation to the adapter only (rejected: directive M requires fail at both layers; the provider must never widen the contract at the boundary).
- 2026-08-19 M4 | Business-binding isolation precedes transport: binding mismatch (wrong tenant/business id vs the adapter's bound business) -> `Authorization` with ZERO transport calls; inactive binding -> `Policy` with zero mutation; adapter availability never overrides business policy. Evidence: `ep028_failure_denied_permission_binding_mismatch` green.
- 2026-08-19 M4 | In-flight idempotency conflict semantics are real concurrency: request A in flight (gated transport blocks), duplicate with the same idempotency identity -> `Conflict` with zero second transport call; after A completes the claim releases and a retry succeeds. Evidence: `ep028_failure_in_flight_duplicate_conflict_and_release` green (Arc<Mutex<bool>> gate + Arc<AtomicUsize> call counter; not a sequential map inspection).
- 2026-08-19 M4 | Bounded recovery after unavailable: transport against a refused port fails `Unavailable` truthfully; a FRESH transport against a healthy controlled fixture then succeeds (`SUBMITTED`), proving no stale failure state poisons the adapter and no stale success fabricates recovery. Evidence: `ep028_failure_bounded_recovery_after_unavailable` green.
- 2026-08-19 M4 | Redaction is poison-safe across real failure paths: `HydraObservability` redacts the credential canary at insert, so a poisoned error (canary embedded in detail/fields) leaves ZERO canary leakage in the audit ring, and the ops diagnostic redacts Bearer/Authorization headers and never prints credentials. Evidence: `ep028_failure_redaction_canary_zero_leakage` green + `hydra-diag.sh` reachable/unreachable runs (no token in output).
- 2026-08-19 M4 | Ops diagnostic fails closed: `tests/hydra/ops/hydra-diag.sh` returns exit 3 (non-zero) for an unreachable endpoint and never prints "healthy" merely because config exists; the probe pipeline's exit status is curl's, not head's. Evidence: `sh tests/hydra/ops/hydra-diag.sh diagnose http://127.0.0.1:1` -> reachable=no, rc=3; reachable controlled fixture -> truthful capabilities, rc=0. Alternatives: masking curl failure with head (rejected: fabricated "healthy" is a failure state under the reality law).
- 2026-08-19 M5 | Live-fire proofs drive the REAL production adapter/transport over REAL sockets against controlled fixtures emitting REAL Hydra-shaped responses (LF-015 hydra-cross-crm-command; LF-025 ceo-business-brief). Evidence is current-run (EP028_M5_RUN_ID bound; stale never satisfies), machine-readable, redacted (canary scan), and records the honest certification boundary (real Hydra provider NOT_ASSERTED). Alternatives: real provider live-fire (rejected: no Hydra component selected in COMPONENT_REGISTRY; DEFERRED with owner), proof-runner placeholders (rejected: EP-001 masking class).
...
- 2026-08-19 M3: Real dependency and transport integration green. Changed files vs fence: schemas/hydra/ (business-context, context-projection, capability-map, action-request, event-envelope JSON Schemas 2020-12), connectors/hydra/tests/ep028_m3_transport.rs (real-socket integration + schema parity under the already-owned connector directory), connectors/hydra/Cargo.toml (jsonschema dev-dep), scripts/ep028-m3-tests.sh (real gate), scripts/nodes/EP-028.sh (M3 branch rewired from the EP-001-masking wrong-crate filter), .agent/milestone-files/EP-028-M3.txt, .agent/expected-files/EP-028.txt (M3 gate registered), Cargo.lock, ExecPlan. Commands + sentinels: `cargo test --locked -p nexus-hydra-connector --all-targets` -> 22 passed 0 failed (11 unit + 11 integration over REAL std::net sockets); `sh scripts/ep028-m3-tests.sh` -> `EP-028 M3: ok`; `sh scripts/nodes/EP-028.sh M3` -> `EP-028 M3: ok` (RC=0); M1 regression 27 green; M2 lib regression green; fmt clean; clippy -D warnings clean; scope audit EP-028: ok; security check: ok; license gate: ok; reality gate: ok; dependency audit: ok; blueprint validation: ok. Certification: canonical Hydra transport surface TRANSPORT_CERTIFIED/PROTOCOL_CERTIFIED over real HTTP against controlled fixtures (EP-026 M3 pattern); schema parity CERTIFIED (Rust serde output validates against schemas/hydra/); real Hydra/CRM provider NOT ASSERTED (no component selected in COMPONENT_REGISTRY; Postiz is EP-029's node; real provider certification DEFERRED with owner recorded). Assumptions confirmed: the versioned canonical surface IS the wire contract (anti-hallucination). Remaining risks: live-fire LF-015/LF-025 and real provider integration owned by M4-M5/deployment.
- 2026-08-19 M4: Forced failures, abuse cases, and observability green. Changed files vs fence: tests/hydra/ (Cargo.toml, src/lib.rs, tests/failure_hydra.rs, ops/hydra-diag.sh), connectors/hydra/src/transport.rs (production fix: fail-closed canonical HydraActionState validation at the transport vocabulary boundary - unknown provider state -> ExternalProvider), scripts/ep028-m4-tests.sh (real gate; fixed the set -e diag_rc capture so the fail-closed assertion actually runs), scripts/nodes/EP-028.sh (M4 branch already rewired), .agent/milestone-files/EP-028-M4.txt (explicit fence per EP-027-M4 precedent), ExecPlan. Commands + sentinels: `cargo test -p nexus-hydra-e2e --all-targets` -> 11 passed 0 failed 0 ignored 0 filtered (refused->Unavailable, silent-peer->Timeout, malformed->External, policy denial ZERO transport calls, binding mismatch->Authorization, in-flight duplicate->Conflict+release, fabricated state->External at transport AND adapter, redaction canary zero leakage, bounded recovery, cancelled work fails closed); `sh scripts/ep028-m4-tests.sh` -> `EP-028 M4: ok` (6 vacuity guards incl. anti-masking ep028_failure_* sentinels + no-ignored guard + ops diagnostic fail-closed rc=3); `sh scripts/nodes/EP-028.sh M4` -> `EP-028 M4: ok` (RC=0); M1/M2/M3 regressions green (26+1 contract, 11 connector unit, 11 M3 integration); fmt clean; clippy -D warnings clean; security check: ok; license gate: ok; reality gate: ok; dependency audit: ok; blueprint validation: ok; scope audit EP-028: ok; orphan audits ok; workspace battery 1943 passed 0 failed 0 ignored (205 suites). Certification: Hydra contract INTERNAL_CERTIFIED (M1); Hydra adapter IMPLEMENTED (M2); Hydra HTTP transport TRANSPORT_CERTIFIED against controlled real-socket fixtures (M3, hardening M4); real Hydra provider NOT ASSERTED (no component selected; DEFERRED owner deployment/M5); Postiz NOT ASSERTED (EP-029 owner); direct database NOT A SUPPORTED CHANNEL (structural). Assumptions confirmed: M4 owns tests/hydra; failure mechanisms are REAL (no mocked transport/adapter; mocks control the peer only). Remaining risks: live-fire LF-015/LF-025 + real provider integration owned by M5/deployment; sandbox/public-registry certification are explicit owned debts (not EP-028-owned).
- 2026-08-19 M5: Live-fire, operations, and node closure green. Changed files vs fence: tests/hydra-live/ (Cargo.toml, src/lib.rs, src/fixture.rs, tests/lf015_cross_crm_command.rs, tests/lf025_ceo_business_brief.rs), scripts/live-fire/LF-015.sh + LF-025.sh (real gates replacing proof-runner placeholders), scripts/ep028-m5-tests.sh (real gate: 9 guards incl. anti-masking LF-015/LF-025 sentinels, current-run evidence run_id match, redaction scan, evidence JSON + certification boundary, M1/M2/M3/M4 regressions), scripts/nodes/EP-028.sh (M5|verify rewired from masking branch to the real gate), docs/operations/EP-028-hydra.md (ops runbook: health/readiness/backup/upgrade/disable/rollback/certification/evidence), .agent/milestone-files/EP-028-M5.txt, .agent/expected-files/EP-028.txt (M5 gate/LF/docs/evidence registered), Cargo.toml (workspace member tests/hydra-live), Cargo.lock, ExecPlan, .agent/state/evidence/ (LF-015/LF-025 current-run JSON). Commands + sentinels: `cargo test --locked -p nexus-hydra-live-e2e --all-targets` -> 2 passed 0 failed (LF-015 + LF-025 over REAL std::net sockets); `EP028_M5_RUN_ID=... sh scripts/ep028-m5-tests.sh` -> `EP-028 M5: ok` (evidence run_id match; redaction clean; M1+M2/M3+M4 regressions green); `EP028_M5_RUN_ID=... sh scripts/nodes/EP-028.sh M5` -> `EP-028 M5: ok` (RC=0); fmt clean; clippy -D warnings clean; security check: ok; license gate: ok; reality gate: ok; dependency audit: ok; blueprint validation: ok (ASCII normalized); prettier clean; scope audit EP-028: ok (tests/hydra-live registered). Certification: Hydra contract INTERNAL_CERTIFIED (M1); Hydra adapter IMPLEMENTED (M2); Hydra HTTP transport TRANSPORT_CERTIFIED against controlled real-socket fixtures (M3/M4/M5); LF-015 + LF-025 live-fire PROVEN over the canonical surface; real Hydra/CRM provider NOT ASSERTED (no component selected; DEFERRED owner deployment); Postiz NOT ASSERTED (EP-029 owner); direct database NOT A SUPPORTED CHANNEL (structural). Assumptions confirmed: the versioned canonical surface IS the wire contract; live-fire uses controlled fixtures because no real provider is selected (anti-hallucination). Remaining risks: real Hydra/Postiz provider integration owned by deployment/EP-029; sandbox/public-registry certification explicit owned debts (EP-040/EP-043).
