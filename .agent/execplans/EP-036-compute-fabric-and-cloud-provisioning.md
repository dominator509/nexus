...
- [x] M4: Forced failures, abuse cases, and observability (2026-08-21): providers/contabo @nexus-provider-contabo Contabo provider binding identity crate (workspace member +1; deps nexus-compute + nexus-domain + serde only, no SDK; canonical ProviderKind::Contabo binding; region code shape validation EU/US/SGP/AUS-style uppercase 1..=4 (exact available set is provider readback data, never hard-coded); opaque CloudCredentialRef binding; no invented Contabo API surface - real Contabo OAuth2 client-credentials + compute instance endpoints owned by later provider-certification milestone) + tests/infra @nexus-infra-failure-tests forced-failure suite (workspace member +1; 7 ep036_failure_* tests with REAL failure mechanisms, no mocked component): ep036_failure_unavailable_dependency_terminated_container (real alpine+openssh ephemeral sshd built + started, then docker rm -f TERMINATED mid-flight, real ssh-keyscan probe must fail - no fabricated reachability, leak assertion), ep036_failure_timeout_probe_fails_closed (real ssh-keyscan -T 1 against dead endpoint never yields a host key), ep036_failure_malformed_input_rejected (runtime-constructed secret-shaped credential canary rejected Validation, empty idempotency key rejected, zero-memory capacity rejected), ep036_failure_duplicate_request_requires_reconciliation (same idempotency key + ambiguous provider outcome -> resolve_ambiguous_provisioning(confirm=false) refuses success, plan stays AMBIGUOUS, never blind retry), ep036_failure_denied_permission_placement_fails_closed (placement across tenant boundary -> PlacementFailureClass::TenantBoundaryViolation, selected_node None, no fallback downgrade), ep036_failure_cancelled_work_delete_before_create_fails_closed (resource ladder never admits leaps; absent-verification before delete-accepted rejected Policy; plan never claims CREATED/READY), ep036_failure_partial_side_effect_receipt_no_overclaim (bare submit never READY/VERIFIED; ambiguous + identity requires provider confirmation; success after reconciliation still needs readback). 10 tests green (7 failure + 3 contabo; zero failed/ignored); clippy -D warnings clean; fmt clean; gate scripts/ep036-m4-tests.sh (roots present, real cargo test --nocapture, non-zero pass + zero failed/ignored vacuity guards, 9 anti-masking sentinels incl. all 7 failure proofs + docker-skip guard, clippy, fmt, M1+M2+M3 regressions); node M4 rewired from phantom tests/infra/test_ep036.py masking to real gate with rc propagation + anti-phantom guard; side gates: scope audit EP-036: ok, preflight: ok, reality gate: ok, security check: ok (M4 RUN requirement), license gate: ok (M4 RUN requirement), dependency audit: ok, blueprint validation: ok, format check: ok, lint: ok, typecheck: ok, test-unit: ok (workspace); certification (honest): Contabo binding identity INTERNAL CONTRACT CERTIFIED, forced-failure suite FAIL-CLOSED BEHAVIOR CERTIFIED (real mechanisms exercised: container termination, real timeout, policy denial, duplicate reconciliation, malformed rejection, delete-chain, partial side effect), structured errors asserted with SPEC-006 codes; real Contabo API / real OAuth token exchange / real instance creation / real cloud account access NOT ASSERTED (later provider-certification milestone owns them; no credentials introduced)
- [x] M5: Live-fire, operations, and node closure (2026-08-21): providers/hetzner @nexus-provider-hetzner Hetzner provider binding identity crate (workspace member +1; deps nexus-compute + nexus-domain + serde only, no SDK; canonical ProviderKind::Hetzner binding; location slug shape validation fsn1/nbg1/hel1/ash/hil-style letter-first lowercase + optional trailing digit, exact available set provider readback data never hard-coded; opaque CloudCredentialRef binding; no invented Hetzner API surface - documented Cloud API v1 token auth + server endpoints owned by later provider-certification milestone). Per the authoritative ExecPlan, no standalone live-fire proof is owned by this node; behavior is exercised by downstream proofs and node-specific REAL dependency tests. M5 gate scripts/ep036-m5-tests.sh runs every node-owned real proof (M1 contract 48, M2 real OpenTofu validate+fmt, M3 real ephemeral sshd transport + ssh-keyscan, M4 real forced-failure suite 7) + hetzner binding suite (3 tests) + clippy -D warnings + fmt + writes machine-readable evidence .agent/state/evidence/EP-036-m5.json (run_id bound, git_commit bound, honest certification boundary: contract/binding INTERNAL CONTRACT CERTIFIED where owned; real cloud APIs/OAuth/instance creation/plan/apply/VM execution/fleet enrollment NOT ASSERTED). Node M5/verify branch rewired from phantom tests/infra/test_ep036.py masking + no-op `:` + unconditional ok tail to real gate with rc propagation + anti-phantom guard. Operations section 15 added: health/readiness/backup/restore/upgrade/disable/rollback for owned artifacts (no long-running service owned). Evidence: node verify chain + scope audit + side gates on committed tree. Certification (honest): Hetzner binding identity INTERNAL CONTRACT CERTIFIED; real Hetzner Cloud API / real token auth / real server creation / real cloud account access NOT ASSERTED (later provider-certification milestone owns them).
...
- 2026-08-21: Side-gate security check caught a real defect in committed M1 source: literal secret-shaped canaries (AWS access-key shaped and PEM private-key shaped) violated the repo runtime-construction convention and only passed at M1 because security-check scans tracked files (the crate was untracked pre-commit). Rewrote the canaries as runtime-concatenated strings; security check green, M1 suite re-run 45 tests green.
...
- 2026-08-21: Node script M4 branch called phantom `tests/infra/test_ep036.py` (never created); rewired to the real M4 gate with rc propagation plus an anti-phantom guard that fails if the phantom path ever reappears.
- 2026-08-21: M4 failure-suite compile fixes: real contract signatures surfaced during build (placement_decision takes request_id/manifest_id/constraint/nodes and returns Result; PlacementFailureClass::TenantBoundaryViolation; ResourceIdentity::new takes 7 args incl. idempotency_key; PlacementConstraint::new takes 11 args with Vec allowed_classes/allowed_regions; VerificationState has Pending/Verified/Mismatch - no Unverified; ComputeClass::Vps not General; Locality::HomeEdge not Home). Fixed against the real API, never weakened.
- 2026-08-21: M4 cid()/tid() test-helper hash overflow: long ids (req-cancel/req-partial) overflowed 48 bits and produced malformed UUIDs; masked with `& 0x0000_ffff_ffff_ffff` so the canonical UUIDv7 test-id shape is preserved (same pattern as M1's strict-UUIDv7 requirement).
- 2026-08-21: Node script M5/verify branch was EP-001 masking class: called phantom `tests/infra/test_ep036.py`, had a no-op `:` branch and an unconditional ok tail; rewired to the real M5 gate with rc propagation plus an anti-phantom guard.
- 2026-08-21: M5 hetzner location slug shape bug caught by real tests: `fsn1`/`nbg1` end in digits so all-lowercase validation rejected valid slugs; fixed to letter-first + optional trailing digits (digit-first strings still rejected). Real tests found the defect, never weakened.
- 2026-08-21: M5 gate evidence total was misleading (nested gate logs are separate files, so only the hetzner count lands in the M5 log); evidence now records per-milestone proof strings instead of a single total.
...
- 2026-08-21: M4 Contabo region codes are shape-validated only (uppercase 1..=4 letters, e.g. EU/US/SGP/AUS); no region catalog is hard-coded and no Contabo API surface is invented - the documented OAuth2 client-credentials flow and compute instance endpoints are owned by the later provider-certification milestone. Evidence: M4 contabo binding tests + no SDK imports.
- 2026-08-21: M4 forced-failure suite proves fail-closed behavior through REAL mechanisms only (terminated container, real ssh-keyscan timeout, corrupted controlled inputs, denied placement policy, duplicate idempotency with provider non-confirmation, delete-chain enforcement, ambiguous partial side effects) - no component being proven is mocked; structured SPEC-006 error codes are asserted. Evidence: 7 ep036_failure_* tests + anti-masking sentinels.
- 2026-08-21: M5 Hetzner location slugs are shape-validated only (letter-first lowercase + optional trailing digit, e.g. fsn1/nbg1/hel1/ash/hil); no location catalog is hard-coded and no Hetzner API surface is invented - the documented Cloud API v1 token auth and server endpoints are owned by the later provider-certification milestone. Evidence: M5 hetzner binding tests + no SDK imports.
- 2026-08-21: M5 writes machine-readable closure evidence (.agent/state/evidence/EP-036-m5.json) bound to the verified git commit with an honest certification boundary; no standalone live-fire is claimed for this node (authoritative ExecPlan section 9), and real cloud provider certification remains NOT ASSERTED. Evidence: M5 gate output + evidence JSON validation.
...
## 15. Operations: health, readiness, backup, restore, upgrade, disable, rollback (M5)

Owned components (provider binding crates, OpenTofu module root, cloud-init config, failure suite, gates) are build-time/contract artifacts; there is no long-running service owned by EP-036. Operations below apply to the owned artifacts and their proofs.

- Health: `sh scripts/nodes/EP-036.sh M5` must print `EP-036 M5: ok` (exit 0). Per-milestone health: `sh scripts/nodes/EP-036.sh M1|M2|M3|M4` each print `EP-036 M<n>: ok`.
- Readiness: `sh scripts/node-verify.sh EP-036` must print `node verify EP-036: ok` on the committed tree; `sh scripts/expected-files.sh EP-036` and `sh scripts/scope-audit.sh EP-036` must print ok.
- Backup: the durable node state is the append-only ledger (`.agent/state/LEDGER.md`) and machine evidence (`.agent/state/evidence/EP-036-m5.json`). Backup = `git push` of the verified commit and green tag; evidence is regenerated by the M5 gate and bound to the verified commit.
- Restore: cold resume = boot sequence (read AGENTS.md/COMMANDS.md/GRAPH.md/LOOPS.md, `sh scripts/ledger.sh tail 30`, `sh scripts/preflight.sh`), confirm the EP-036 lease, then rerun the last checked milestone sentinel on the committed tree.
- Upgrade: apply later milestone commits only after their own committed-tree reproduction; never cross a completed green tag (LOOPS.md rollback rule).
- Disable: provider bindings are declarative crates; disabling a provider means not registering it in the fabric (no runtime service to stop). The ephemeral sshd fixture tears itself down in all exit paths; the throwaway image is removed after the M3/M4/M5 gates.
- Rollback: `git revert`/checkout to the previous milestone commit per LOOPS.md; never roll back across `green/EP-036`.
