...
- 2026-08-15 | Discovery: M5/LF-016 development found the M2 design promise ("the production implementation runs real CLI processes; tests inject a scripted runner") was never fulfilled: only the CONTROLLED_TEST_FIXTURE `ScriptedRunner` existed and `scripts/live-fire/LF-016.sh` delegated to a nonexistent nexus-cli `proof run` (the EP-015 LF-021 dead-delegation pattern). Built the REAL production transport `ProcessRunner` (`crates/nexus-harness-adapters/src/process.rs`) that spawns real subprocesses per normalized command with capped output capture and SPEC-006 exit mapping (Success/Failure(code)/Unavailable on spawn failure), wired it into the crate public surface, created the CONTROLLED_TEST_FIXTURE executable `tests/agents/fixtures/coding-agent-fixture.sh`, rewrote LF-016.sh as a direct real proof (EP-015 precedent), and proved the full cowork chain through the REAL process boundary (5 lf016 tests). External provider certification (Codex/Claude/Hermes/OpenClaw CLIs) is DEFERRED with owner recorded in the certification registry: none of those CLIs are installed and no provider credential exists in this environment; the fixture is explicitly CONTROLLED_TEST_FIXTURE and never claims real provider behavior.
...
- 2026-08-15 | Decision: Implement the EP-017 M5 live-fire and closure with the REAL production process transport. (1) The M2 design promised a production `HarnessCommandRunner` that runs real CLI processes, but only the CONTROLLED_TEST_FIXTURE `ScriptedRunner` existed. Built `ProcessRunner` (`crates/nexus-harness-adapters/src/process.rs`): spawns a real subprocess per normalized command, captures stdout/stderr to a capped buffer (1 MiB, hostile harness cannot exhaust memory), maps exit status to the SPEC-006 transport vocabulary (exit 0 -> Success, non-zero -> Failure(code), spawn failure -> typed Unavailable). Exported `HarnessCommandKind::as_str()` and the new `ProcessRunner` from the crate surface. (2) LF-016 (`coding-agent-cowork`) was rewritten from the dead nexus-cli `proof run` delegation (EP-015 LF-021 pattern) to a direct real proof: `crates/nexus-harness-adapters/tests/lf016.rs` drives the production composition (DeterministicAgentRegistry + TaskOrchestrator + CliHarnessAdapter + ProcessRunner) against the REAL executable `tests/agents/fixtures/coding-agent-fixture.sh` (CONTROLLED_TEST_FIXTURE), proving the full cowork chain: task -> capability selection -> budget-bound assignment -> real process execution -> progress -> artifact exchange -> bounded review -> completion/delegation COMPLETED; cancellation through the real transport (delegation REVOKED, no orphan); nonzero exit fails closed (typed UNAVAILABLE, task stays ASSIGNED); exit-status mapping; missing executable fails closed. (3) External provider certification (Codex / Claude Code / Hermes / OpenClaw CLIs) is DEFERRED: none are installed and no provider credential exists; the fixture is explicitly CONTROLLED_TEST_FIXTURE and never claims real provider behavior. Registry row `agent-harness-adapters` added with owner EP-043 (production readiness and ship; external provider certification owner to be confirmed at ship-gate review). Evidence: `EP-017 M5: ok`, `LF-016: ok` (5 lf016 tests), `node verify EP-017: ok` with MANDATORY `runtime smoke: ok` against the real EP-044 control-plane container (http://127.0.0.1:8443 /healthz /readyz /v1/capabilities), `scope audit EP-017: ok`, expected files ok, security/license/reality/format/lint/dependency ok, evidence `.agent/state/evidence/LF-016-ep017-m5.md`. Alternatives: leave LF-016 as the dead nexus-cli delegation (rejected: it would have failed at M5 gate with a fabricated success claim); use ScriptedRunner for the live-fire (rejected: LF-016 must prove the real process boundary, not a fixture transport); claim real provider certification (rejected: no CLIs/credentials exist). Consequence: EP-017 owns a real, proven process boundary for harness adapters; external coding-agent providers remain DEFERRED with explicit owner; node verify proves the mandatory runtime smoke. Reversal: revert M5 commit. Security: capped output capture; typed SPEC-006 errors; redacted messages; fixture is CONTROLLED_TEST_FIXTURE and never claims provider behavior. License: no new dependency classes (std only). Compatibility: additive production transport + fixture; existing 20 ep017_unit + 35 ep017_failure tests unchanged and green.

# 14. Outcomes & Retrospective

EP-017 complete (Agent Orchestrator and Harness Adapters; SPEC-010; ADR-024).

Changed vs machine fence: crates/nexus-agents (M1), crates/nexus-harness-adapters (M2/M4/M5 + process.rs), packages/workflows/src/agents/ (M3), tests/agents/ (M4 README + M5 fixture), scripts/nodes/EP-017.sh (M1-M4 gate vacuity fixes), scripts/ep017-m3-tests.sh (M3 gate), scripts/live-fire/LF-016.sh (rewritten from dead nexus-cli delegation), .agent/state/evidence/CERTIFICATION_REGISTRY.md (agent-workflow-contracts + agent-harness-adapters rows), .agent/state/evidence/LF-016-ep017-m5.md, references/ADR-024, docs/vocabulary/README.md, Cargo.toml/Cargo.lock.

Exact commands and observed sentinels: `sh scripts/nodes/EP-017.sh M1` -> `EP-017 M1: ok` (19 ep017_unit); M2 -> `EP-017 M2: ok` (20 ep017_unit, 4 suites); M3 -> `EP-017 M3: ok` (10 ep017_integration vitest + tsc clean); M4 -> `EP-017 M4: ok` (35 ep017_failure); M5 -> `EP-017 M5: ok`; `sh scripts/live-fire/LF-016.sh` -> `LF-016: ok` (5 lf016 real-process tests); `sh scripts/node-verify.sh EP-017` -> `node verify EP-017: ok` with `runtime smoke: ok` (MANDATORY, real EP-044 control-plane container at http://127.0.0.1:8443: /healthz healthy, /readyz ready, /v1/capabilities non-empty); `scope audit EP-017: ok`; `security check: ok`; `license gate: ok`; `reality gate: ok`; `format check: ok`; `lint: ok`; `dependency audit: ok`.

Test and proof evidence: 19 + 20 + 10 + 35 + 5 = 89 EP-017-owned tests green; two real production defects fixed at M4 (cancel_task orphan + cross-task artifact); real process transport built at M5 (ProcessRunner) proving the real subprocess boundary end to end.

Assumptions confirmed/changed: M3 owns workflow implementation only (real Temporal engine integration owned by EP-006 substrate; registry row records this); external coding-agent providers (Codex/Claude/Hermes/OpenClaw) are DEFERRED with owner EP-043 - no CLIs installed, no credentials, fixture is CONTROLLED_TEST_FIXTURE.

Provider and hardware status: deepseek-reflex PROVIDER_CERTIFIED (EP-014); bifrost INTERNAL_CERTIFIED; external coding-agent providers DEFERRED; no hardware certification owned by this node.

Remaining risks: real Temporal execution of agent workflows and real external coding-agent provider certification are both deferred with explicit owners recorded in the certification registry; neither blocks this node's completion (blocking_for_ship: false) but both must be resolved at ship-gate review.

Green tag: green/EP-017 (created at node closure).
