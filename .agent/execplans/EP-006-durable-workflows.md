...
- 2026-08-13: M5 preflight defect 2: `scripts/ep006-orphan-audit.sh` used docker Go-template `--format` with `.Names` / `.Name` field accessors (double-brace syntax); the blueprint validator treats double-brace as an unresolved placeholder in non-code files (.sh is not in CODE_EXTS). Rewrote the two queries to parse docker's plain table with awk (`$NF` / `$2`), preserving exact behavior (verified `EP-006 orphan audit: ok`).
