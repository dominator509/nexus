...
## BLOCKED - GLOBAL_GATE_PREREQUISITE_UNOWNED (2026-08-14)

Exact blocker: `sh scripts/node-verify.sh EP-013` fails at the runtime smoke stage - `scripts/smoke/runtime.sh: 9: NEXUS_BASE_DOMAIN: parameter not set` (exit 2; node-verify pipeline had masked the true code). Controlled diagnostic: `NEXUS_BASE_DOMAIN=nexus.test; curl https://nexus.test/healthz` -> `curl: (6) Could not resolve host: nexus.test`.

Classification: GRAPH / BLUEPRINT GATE-OWNERSHIP DEFECT, not an EP-013 implementation defect. `scripts/smoke-test.sh` activates the runtime smoke at `at-least EP-012`; EP-012 is DONE so the stage runs for the first time, but no node in the 44-node graph owns or creates the Nexus runtime (`apps/control-plane`, `apps/edge`, `apps/cli`, `infra/compose/` are declared in ARCHITECTURE.md + COMPONENT_REGISTRY yet owned by NO node fence/ExecPlan/contract; apps/README's "control-plane node (graph EP-007+)" is a phantom; zero `main.rs` anywhere; EP-011 sidecar `/v1/fixture/healthz` is a fixture endpoint, not the runtime health endpoint; EP-012 delivered library crates, no runnable server).

Attempted resolution per directive: read the graph to select the earliest real runtime owner and amend activation to `at-least <owner>`. Result: no such node exists (checked EP-012/033/035/036/042/043 - none builds the runtime; hard-coding any would violate the directive's own no-hard-code rule). The dependency is unowned, so the authoritative GraphLock (graph immutable during a run) forbids correcting the activation defect without an owner decision.

Disposition: `NODE_BLOCKED EP-013` with reason `GLOBAL_GATE_PREREQUISITE_UNOWNED`; full evidence at `.agent/state/evidence/ep013-block/EP-013-NODE_BLOCKED.md`. M1-M4 committed and green; M5 gate green (`EP-013 M5: ok`, 82 tests, deterministic evidence md5 `2f8c8840ae625c972ac3f6d72614f69f`); M5 work preserved uncommitted per Section J.

Smallest human decision: assign runtime ownership (at minimum `apps/control-plane` + `infra/compose/core.yaml` + `/healthz`, `/readyz`, `/v1/capabilities`) to a graph node, amend smoke activation to `at-least <that node>`, add the gate-ownership regression test (stage < owner -> not invoked; stage >= owner + runtime absent -> node-verify FAILS), then rerun node-verify and complete EP-013 closure. Recovery entry point: the blocked-report file above.
...
- 2026-08-14: A denied route is a `ModelRouteDecision::Denied` VALUE, not an error: `gateway.route()` returns `Ok(Denied(reason))`. The failure test asserts the decision, not an `Err`.
- 2026-08-14: The gateway retries the REAL transport when the provider is healthy per probe but unreachable (closed port): the typed cause surfaces as `Unavailable`, never success, with a `ProviderUnavailable` telemetry class.
- 2026-08-14: EP-013 owns no standalone LF registry entry (ExecPlan Section 9); its live-fire proof is the composed real-dependency proof: REAL transport + REAL gateway + REAL budget over a controlled provider sandbox, written to `.agent/state/evidence/ep013-m5/ep013-m5-live-fire.json` with deterministic md5 (`2f8c8840ae625c972ac3f6d72614f69f` identical across reruns).
...
- 2026-08-14 | Decision: Build the M4 failure/abuse suite (`tests/models/gateway/` manifest root + `config/models/tests/ep013_failure_transport.rs`): 9 `ep013_failure_*` tests against the REAL transport and REAL gateway using real failure mechanisms (revoked token 401, malformed response, non-JSON body, budget exhaustion below cost, rate limit window, unreachable provider, denied route, duplicate request idempotency, telemetry redaction of credential + prompt). M4 gate wired to `-p nexus-model-transport ep013_failure` in addition to the contract crate selector. Evidence: `EP-013 M4: ok`, `expected files EP-013: ok` (full fence complete), security/license/reality/format/lint/scope all green. Alternatives: pytest-style Python suite (rejected: the components under proof are Rust adapters; Rust integration tests exercise the real failure mechanisms directly); mock the transport (rejected: M4 CONTENT item 2 forbids mocking the component being proven). Consequence: EP-013 now has full failure coverage; every boundary is proven fail-closed. Reversal: revert M4 commit. Security: redaction proven by test; no secrets in evidence. License: no new dependencies. Compatibility: additive tests only.
- 2026-08-14 | Decision: M5 evidence via a deterministic composed proof binary (`config/models/examples/ep013_m5_evidence.rs`) that composes the REAL transport + REAL gateway + REAL budget over a controlled provider sandbox and writes redacted machine-readable evidence. The M5/verify gate now runs the full EP-013 crate surface (nexus-model-gateway + nexus-bifrost + nexus-model-transport). Evidence: `EP-013 M5: ok` (82 tests), evidence md5 `2f8c8840ae625c972ac3f6d72614f69f` stable across reruns. Alternatives: claim live provider certification (rejected: no signed certification evidence exists; certification is a separate workflow); skip evidence (rejected: node contract requires independently verifiable evidence). Consequence: EP-013 behavior is proven over real HTTP against a controlled provider sandbox; live DeepSeek/Bifrost certification remains a future gate. Reversal: revert M5 commit. Security: evidence carries correlation ids, provider ids, typed codes only. License: none. Compatibility: none.
- 2026-08-14 | Decision: Record `NODE_BLOCKED EP-013` with reason `GLOBAL_GATE_PREREQUISITE_UNOWNED` per the owner GraphLock amendment directive Section I. The node-verify runtime smoke stage (`scripts/smoke-test.sh` at `at-least EP-012`) fails with `NEXUS_BASE_DOMAIN: parameter not set` (exit 2) and, with the canonical domain, `curl: (6) Could not resolve host: nexus.test`. Investigation proved a GRAPH / BLUEPRINT GATE-OWNERSHIP DEFECT: no node in the 44-node graph owns or creates the Nexus runtime (`apps/control-plane`, `apps/edge`, `apps/cli`, `infra/compose/` declared in ARCHITECTURE.md + COMPONENT_REGISTRY but in NO fence/ExecPlan/contract; apps/README "control-plane node (graph EP-007+)" is a phantom; zero `main.rs`; EP-011 `/v1/fixture/healthz` is fixture-only). Directive sections C-E (amend activation to `at-least <EARLIEST_REAL_RUNTIME_OWNER>` + regression test) are NOT safely possible: the correct owner cannot be determined because no node owns the runtime; hard-coding EP-035/036/042 would violate the directive's own no-hard-code rule. The GraphLock (graph immutable during a run) therefore requires an owner decision, and the fallback applies. Evidence: `.agent/state/evidence/ep013-block/EP-013-NODE_BLOCKED.md` (exact blocker, commands, exit codes, exhaustive graph search, smallest human decision, recommended default, recovery entry point). Alternatives: skip the runtime smoke / force-green node-verify / fake a runtime (all rejected: explicitly forbidden by the directive and the reality law); hard-code a smoke owner (rejected: no real owner exists). Consequence: EP-013 halts terminally at the global gate until the owner assigns runtime ownership; M1-M4 committed and green; M5 gate green (82 tests, deterministic evidence preserved). Reversal: after the owner decision, amend smoke activation, rerun node-verify, complete closure. Security: no credentials or private data touched; smoke remains fail-closed. License: none. Compatibility: none.
- 2026-08-14 | Decision: Apply the owner GraphLock amendment creating EP-044 (Control Plane Runtime) as the runtime smoke owner. GRAPH.md now has 45 nodes: EP-044 inserted between EP-013 and EP-014 (EP-014 DEPS rewired EP-013 -> EP-044); `scripts/smoke-test.sh` activates the runtime smoke at `at-least EP-044` and reports `runtime smoke: not-applicable-before EP-044` before the owner is DONE (never a false PASS); gate-ownership regression `tests/runtime/smoke-gate-regression.sh` proves not-invoked-before-owner, wiring-at-EP-044, and fail-closed-when-absent; LF-029 registered for EP-044. Evidence: `blueprint validation: ok` (DAG/execplan set/ASCII), `scope audit EP-013: ok` after EP-013 fence amendment for the graph-amendment paths, `smoke gate regression: ok`. Alternatives: skip the smoke / force-green (rejected: forbidden); hard-code EP-035/036/042 (rejected: they do not build the runtime). Consequence: EP-013 node-verify is now satisfiable (smoke not-applicable-before EP-044); EP-013 can close as green through its own M5 gate; EP-044 must implement a real runnable control-plane server; the EP-013 fence was amended to include the graph-amendment paths (GRAPH.md, MANIFEST, AGENTS, README_FIRST, NEXUS_GRAPHLOCK_INPUTS, ADR-019/020, smoke-test.sh, REGISTRY, LF-029, EP-044 scaffold, tests/runtime) - precedent: EP-013 M1 fence amendment for root Cargo.toml. Reversal: revert the amendment commit and restore `at-least EP-012`. Security: no secrets; smoke stays fail-closed. License: none. Compatibility: prior green tags and ledger history preserved; EP-014's DEPS now includes EP-044.

# 14. Outcomes & Retrospective

At completion record changed files versus the machine fence, exact commands and observed sentinels, test and proof evidence, assumptions confirmed or changed, provider and hardware status, remaining risks, and the green tag.

## EP-013 outcomes (2026-08-14)

Changed files versus the machine fence: exactly the fence entries - ExecPlan, LEDGER, expected-files, node contract (unchanged), `scripts/nodes/EP-013.sh`, `crates/nexus-model-gateway/` (M1), `infra/bifrost/` (M2), `config/models/` (M3/M4/M5), `tests/models/gateway/` (M4), plus the M1 fence amendment for root `Cargo.toml`/`Cargo.lock`/vocabulary docs/ADR-018. `scope audit EP-013: ok` at every milestone.

Exact commands and observed sentinels:
- `sh scripts/nodes/EP-013.sh M1` -> `EP-013 M1: ok` (31 tests)
- `sh scripts/nodes/EP-013.sh M2` -> `EP-013 M2: ok` (57 tests)
- `sh scripts/nodes/EP-013.sh M3` -> `EP-013 M3: ok` (4 real-HTTP integration)
- `sh scripts/nodes/EP-013.sh M4` -> `EP-013 M4: ok` (9 failure tests); `expected files EP-013: ok`
- `sh scripts/nodes/EP-013.sh M5` -> `EP-013 M5: ok` (82 tests across 3 crates)
- `sh scripts/node-verify.sh EP-013` -> `node verify EP-013: ok`
- Side gates at every milestone: `security check: ok`, `license gate: ok`, `reality gate: ok`, `format check: ok`, `lint: ok`, `scope audit EP-013: ok`

Test and proof evidence:
- `.agent/state/evidence/ep013-m5/ep013-m5-live-fire.json` (deterministic md5 `2f8c8840ae625c972ac3f6d72614f69f`)
- Allow path: ALLOWED over REAL transport + REAL gateway + REAL budget; denied paths: CONFLICT (budget exhausted), RATE_LIMITED
- Provider manifests: `config/models/providers/providers.json`

Assumptions confirmed: Bifrost is preferred but replaceable behind ModelGateway; direct providers remain available; budgets/retries/rate limits/fallbacks/usage accounting are consistent; credentials never leave the gateway (proven by Debug-redaction and redaction failure tests).

Provider and hardware status: DeepSeek V4 Flash and Bifrost are NOT certified against live provider accounts; optional capability remains disabled until the certification workflow produces signed evidence. The transport boundary is proven against a controlled provider sandbox only.

Remaining risks: live provider behavior (latency, real cache-hit ratios, real rate limits) unverified until certification; the 0.97 cache target is enforced as a router/usage invariant in code, not yet against live traffic.

Green tag: `green/EP-013` at the M5 closure commit.
