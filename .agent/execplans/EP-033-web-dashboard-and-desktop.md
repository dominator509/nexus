...
### M5 completion (2026-08-21)

- `tests/accessibility/web/` @nexus/web-a11y package created: real-browser accessibility scan + LF-005 cross-device continuity proof composed from the REAL production components (@nexus/web contracts, @nexus/desktop runtime/dispatcher/approval-flow, @nexus/ui components rendered through REAL react-dom/server). No mocks; the browser runtime is REAL system Chrome (`/usr/bin/google-chrome`, headless, no emulation) driven by playwright-core, and the scan engine is REAL axe-core injected into the real page.
- Three test files / 13 tests: `ep033_a11y_unit` (11: harness renders owned production surfaces into a complete document, FOUR_EYES requirement verbatim, visible-but-unauthorized disabled, stale status non-color, skip link + main landmark, LF-005 voice-start->web-dashboard correlation, mobile FOUR_EYES two distinct principals, final artifact same task graph, UI authority distinctions DISPLAYED != CURRENT != AUTHORIZED / APPROVED != EXECUTED, current-run identity, dependency-direction), `ep033_a11y_browser_scan` (1: axe-core WCAG 2.2 A/AA tag set wcag2a/wcag2aa/wcag21a/wcag21aa/wcag22aa in REAL Chrome -> 0 violations, 26 passes, skip link + main landmark + focusable elements, writes current-run evidence), `ep033_lf005_evidence` (1: writes LF-005 journey evidence with fresh run_id for the gate to observe).
- Evidence (current-run bound, machine-readable): `.agent/state/evidence/LF-005-ep033-m5.json` (browser scan: axe-core version, browser version, violations [], passes 26, keyboard stats, standard WCAG 2.2 A/AA, scope = owned surfaces DashboardShellView/ApprovalCardView/StatusBadge/CapabilityButton/ChatComposer) and `.agent/state/evidence/LF-005-ep033-m5-lf005.json` (journey: voice_start AGENT transcript origin, web_dashboard_continue objective_bound + rendered_surface objectives, mobile_approval FOUR_EYES satisfied two distinct approvers state APPROVED, final_artifact_same_graph objective_ids + correlation consistent, authority_distinctions true).
- LF-005 runner rewired: `scripts/live-fire/LF-005.sh` previously delegated to `proof-runner.sh` -> `nexus-cli` proof-runner which DOES NOT EXIST in this repo (no such crate). Rewired to call the real gate `scripts/ep033-m5-tests.sh` directly (EP-031 LF-009 precedent). No fake nexus-cli created.
- Node script M5/verify rewired from the masking branch (`pnpm --filter @nexus/web test` = web unit suite + dangling LF-005) to the real gate `scripts/ep033-m5-tests.sh` (guards: package + owned source/test files present, production-import guard @nexus/ui/@nexus/web/@nexus/desktop + no vi.mock, tsc --noEmit clean, non-zero 13 passing, zero failures, zero skipped, three a11y test-file sentinels, 13 exact owned proof-name sentinels, evidence freshness guards - files exist, `"violations": []`, node/milestone bound, mmin -10 fresh, axe-core engine + non-zero passes recorded, LF-005 runner integrity (calls the real gate, no proof-runner.sh), M1+M2+M3+M4 regressions, orphan guard, fence artifacts).
- Side gates: scope audit EP-033: ok; expected files EP-033: ok (fence gained tests/accessibility/web/, scripts/ep033-m5-tests.sh, scripts/live-fire/LF-005.sh, .agent/milestone-files/EP-033-M5.txt); reality gate: ok; license gate: ok; security check: ok 0 advisories; dependency audit: ok; blueprint validation: ok; pnpm -r typecheck: ok (9 packages); pnpm -r --workspace-concurrency=1 test:unit: ok (9 packages: 4+109+143+66+26+42+22+13+26 = 451 passed, 0 failed).
- Commands + sentinels: `sh scripts/ep033-m5-tests.sh` -> EP-033 M5: ok (13 tests); `sh scripts/nodes/EP-033.sh M5` -> EP-033 M5: ok (RC=0).
- Certification (final honest): @nexus/web-a11y accessibility scan + LF-005 continuity proof INTERNAL CONTRACT CERTIFIED; WCAG 2.2 A/AA scan machine-observed in real Chrome with real axe-core (0 violations on owned surfaces); browser runtime REAL (system Chrome, headless, no emulation) TRANSPORT/PROTOCOL CERTIFIED for the exact exercised path; LF-005 cross-device continuity journey COMPOSITION CERTIFIED (real production components; voice start simulated as canonical transcript origin AGENT, mobile approval = desktop approval flow with two distinct principals - no real phone/carrier involved); Tauri desktop shell NOT ASSERTED (thin signed wrapper fallback remains future work); full app-store/PWA deployment NOT ASSERTED.

### M4 completion (2026-08-20)

- `tests/e2e/web/` @nexus/web-e2e forced-failure package over the REAL production components (@nexus/web contracts, @nexus/desktop runtime/dispatcher/approval-flow/telemetry/prefs; @nexus/ui components are covered by the M3 gate regression). Failure mechanisms are controlled inputs and real state transitions, never mocks: corrupted wire payloads, denied policy, duplicate requests, revoked/expired sessions, unavailable connectivity, context switches, secret-leak attempts.
- Four failure files / 26 tests: `ep033_failure_malformed_input` (5: deny-unknown session field, corrupted enum -> Vocabulary, fabricated capability class -> Vocabulary, fabricated capability id -> Vocabulary before dispatch, undersized idempotency key -> Validation), `ep033_failure_denied_policy` (4: R3 without human approval -> Policy, R4 under POLICY -> Policy, revoked session -> Authorization, expired session refused twice - no blind replay), `ep033_failure_state_authority` (9: business-switch projection invalidation, backend-unavailable consequential refusal, offline consequential refusal, offline payload labeled STALE and never actionable, duplicate request executed exactly once via idempotency ring, reused key for different action -> Conflict, four-eyes one-principal-twice abuse -> Conflict/PENDING, fabricated approval class -> Vocabulary, hostile text in command arguments cannot mint authority), `ep033_failure_content_prefs_telemetry` (8: hostile command-like chat text inert, hostile chat never changes session/capability/approval/risk, theme/layout/view changes never touch security state, serialized preference blobs cannot overwrite security keys, persistence boundary refuses secret-marker values by content, desktop telemetry strips bearer/JWT canaries, telemetry strips secret-key/api-key canaries, redacted logger never records private body content).
- Two stale `DesktopDispatcher` references in the denied-policy file (lines ~105/~119) fixed to `DesktopCommandDispatcher` (the import was already correct); `rg '\bDesktopDispatcher\b'` across tests/e2e/web apps/desktop apps/web packages/ui returns zero.
- Node script M4 rewired from the masking `pnpm --filter @nexus/web exec vitest run -t ep033_failure` branch (zero-match filter class - the web package has no ep033_failure tests) to the real gate `scripts/ep033-m4-tests.sh` (guards: package + four failure files present, production-import guard for DesktopCommandDispatcher/DesktopShellRuntime/DesktopApprovalFlow/DesktopTelemetry + no vi.mock, tsc --noEmit clean, non-zero 26 passing, zero failures, zero skipped, four failure-file sentinels, 20 exact owned proof-name sentinels via --reporter=verbose (default reporter omits individual names), M1+M2+M3 regressions, orphan guard, fence artifacts).
- Side gates: to be recorded from the post-commit battery (M4 gate itself runs M1/M2/M3 regressions inline; workspace battery and remaining side gates run after commit).
- Commands + sentinels: `cd tests/e2e/web && pnpm test:unit` -> 4 files / 26 passed (observed); `sh scripts/ep033-m4-tests.sh` -> EP-033 M4: ok (26 tests, observed from gate run); `sh scripts/nodes/EP-033.sh M4` -> EP-033 M4: ok (RC=0, observed from node run).
...
- 2026-08-20 M4: Two stale `DesktopDispatcher` references existed in the M4 denied-policy test file (the import was already correct); `rg '\bDesktopDispatcher\b'` across the whole fence now returns zero. TypeScript LSP caught the stale names before the typecheck.
- 2026-08-20 M4: vitest's DEFAULT reporter does not print individual test names, only per-file pass counts; the gate's anti-masking proof-name greps required `--reporter=verbose` to observe the 20 exact owned proof names.
- 2026-08-20 M4: `PreferencePersistence` classifies by secret MARKERS in value content (bearer/token/secret/...), not by shape - a JWT-shaped canary without a marker is allowed; the test was corrected to use a bearer canary which the production boundary is designed to refuse.
- 2026-08-20 M4: The M4 gate's M1/M2/M3 regression chain (M4 -> M1 -> M2 -> M3, each nesting its predecessors) is CPU-bound under heavy host load (load average 38-66 from two runaway LSP tsserver processes pegged for days); the chain completes in ~2-3 minutes under idle load but must be run in the background with generous timeout when the host is loaded. The gate itself is correct; this is environmental, not a regression.
- 2026-08-21 M5: The evidence JSON records axe violations as an empty array (`"violations": []`) not `0`; the first M5 gate run failed on the `"violations": 0` grep - fixed to `\[\]`.
- 2026-08-21 M5: `scripts/live-fire/LF-005.sh` delegated to `proof-runner.sh` which requires a `nexus-cli`/`nexusctl` proof registry that does not exist (no crate named nexus-cli/nexusctl); rewired to call the real gate `scripts/ep033-m5-tests.sh` directly, mirroring the EP-031 LF-009 precedent. No fake nexus-cli was created.
- 2026-08-21 M5: Plain `node dist/...` cannot resolve workspace TS sources at runtime; the browser scan and LF-005 evidence writer run through vitest's transform pipeline instead (same pattern as M3 SSR integration).
- 2026-08-21 M5: vitest is not linked at the repo root `.bin` for the a11y package; the package-local `./node_modules/.bin/vitest` must be executed directly (never `node <that path>` - it is a shell shim).
- 2026-08-21 M5: The LF-005 "mobile approval" leg uses the production desktop approval flow with two distinct principals; voice start is the canonical transcript-origin AGENT seam - no real phone/carrier is involved, and this is stated in the certification boundary.
...
- 2026-08-20 M4: M4 package boundary = `tests/e2e/web/` forced-failure e2e package (real production components composed, controlled inputs as the failure mechanism - NO mocks, NO M4-only fake dispatcher/session authority). Evidence: 26 tests / 4 files over @nexus/web contracts, @nexus/desktop runtime/dispatcher/approval-flow/telemetry/prefs; alternatives: mock-based failure injection (rejected: directive R requires real components and real failure mechanisms); jsdom/browser e2e (rejected: browser e2e is M5-owned with Playwright/axe; M4 proves contract-level fail-closed semantics deterministically); consequence: every M4 proof exercises production code paths; reversal: none; security: hostile content/preference/telemetry canaries prove data never mints authority and secrets never leak; license: none new (vitest/typescript already in workspace); compatibility: pnpm-workspace.yaml + pnpm-lock.yaml updated (+37 lines) for tests/e2e/web importer.
- 2026-08-21 M5: M5 package boundary = `tests/accessibility/web/` real-browser accessibility scan + LF-005 cross-device continuity proof. Real tooling: playwright-core drives REAL system Chrome (headless, no emulation); axe-core (4.13.0) runs the WCAG 2.2 A/AA tag set against the REAL react-dom/server-rendered markup of the production @nexus/ui surfaces. Evidence: 13 tests / 3 files + current-run machine-readable evidence (.agent/state/evidence/LF-005-ep033-m5.json, LF-005-ep033-m5-lf005.json); alternatives: jsdom/aXe in-process scan (rejected: not machine-observed in a real browser), Playwright browser download (rejected: playwright-core + system Chrome avoids the network browser download and uses the real installed browser), static WCAG claim from unit tests (rejected: directive AI requires machine-observed results); consequence: the scan proves 0 axe violations + 26 passes on owned surfaces in a real browser; reversal: none; security: evidence contains only safe fields (run_id, counts, engine versions), no tokens or body content; license: axe-core MPL-2.0 + playwright-core MIT added to the a11y package devDependencies (license gate ok); compatibility: pnpm-workspace.yaml + pnpm-lock.yaml updated (+2 packages) for tests/accessibility/web importer.
- 2026-08-21 M5: LF-005 runner rewired to a real gate instead of the dangling proof-runner. `scripts/live-fire/LF-005.sh` previously ran `proof-runner.sh LF-005` which requires `nexus-cli`/`nexusctl` (no such crate exists). Rewired to `sh scripts/ep033-m5-tests.sh` (EP-031 LF-009 precedent: live-fire script = real milestone gate). Evidence: gate LF-005 runner-integrity guards (calls the real gate, no proof-runner.sh); alternatives: creating a minimal `nexus-cli` proof-runner to satisfy the old script (rejected: directive AF - no fake nexus-cli); consequence: LF-005 now genuinely executes the cross-device continuity proof and accessibility scan; reversal: none; security: none; license: none; compatibility: none.
