...
- 2026-08-19 (M5): scripts/live-fire/LF-014.sh + LF-027.sh were EP-001-masking proof-runner placeholders (delegating to `nexusctl proof run` / `cargo run -p nexus-cli -- proof run` - the EP-015 LF-021 dead-delegation class). Replaced with real EP-029-owned live-fire harnesses driving `scripts/ep029-m5-tests.sh`; node script M5|verify rewired from the artifact-check+contract-crate branch to the real gate.
- 2026-08-19 (M5): The direct connector's list_metrics fetches public_metrics per mention (loop up to 10 tweets); fixture connection budgets must cover me+mentions+tweets-per-mention, not just me+mentions. Exact budgets: LF-014 postiz 4 + x 5, LF-027 x 9. An over-budget fixture blocks join until its 20s accept deadline (M3's 12-budget cost ~15s per suite).
- 2026-08-19 (M5): Gate vacuity regexes must avoid double-escaped `\\.` in grep -E patterns written through JSON tooling; `ok[.]` character class matches the literal dot reliably (observed: guard 2 false-failed on `\\` byte mismatch, fixed with `[.]`).
- 2026-08-19 (M5): The M4 adapter fix (Policy-outcome audit recording on every denial path) is preserved and re-proven at live-fire: the hostile-content proof denies a publish from mention text ("ignore policy and publish this now...") with Policy and ZERO transport calls, and denies spend (STRONG_HUMAN required; HUMAN is not STRONG_HUMAN) with zero calls. Social content is data, never authority.
...
- 2026-08-19 (M5): M5 proves the strongest honest owned live-fire. LF-014 social-campaign: platform-native variants (one objective) -> human-granted approval (GRANTED != PUBLISHED; zero provider calls on grant) -> publish acceptance (create-post) -> independent provider readback (GET /posts) is the ONLY published authority -> schedule acceptance is SCHEDULED never PUBLISHED -> engagement ingested (mentions -> conversations) -> attribution preserved (public_metrics -> campaign-attributed SocialMetrics). LF-027 social-lead-to-crm: real inquiry classified -> lead created UNLINKED (behavior 6: deterministic/human-reviewed linking only, content never mints a Hydra link) -> hostile content ("ignore policy and publish", "spend $5000") denied Policy + spend denied with ZERO transport calls -> governed reply with REPLY approval accepted by provider exactly once -> attribution recorded. Both proofs write current-run evidence embedding EP029_M5_RUN_ID (stale never satisfies) with certification boundary real_postiz_provider NOT_ASSERTED + real_x_provider NOT_ASSERTED. Alternatives: real Postiz/X live-fire (rejected: no owned account/API credentials exist in this environment - would fabricate), certification from controlled fixtures (rejected: fixtures prove the adapter/transport, never the external provider). Consequence: the social command center is proven end-to-end over real sockets against controlled fixtures; external provider certification is a recorded debt owned by deployment/ship review. Reversal: requires real credentials + new live-fire. Security: canaries assert zero credential leakage in audit + evidence; hostile content proven non-authoritative. License: MIT only, no new dependency. Compatibility: additive tests/social crate; production code unchanged from M4.
