...
- `sh scripts/ep007-keycloak-diag.sh` - one-shot Keycloak 26.7.0 diagnostic and bounded recovery for the EP-007 auth integration: verifies the pinned image digest, imports the Nexus realm into an ephemeral container, waits for the REAL OIDC discovery document (`issuer`), then removes the container. Fails with a clear message if the image is missing or discovery is not ready within 90s. Registered in the EP-007 M4 Decision Log.
- `sh scripts/ep007-orphan-audit.sh` - post-live-fire orphan audit for EP-007 M5: asserts zero `nexus-ep007-*` containers/networks, zero ceremony/browser helper processes, and zero leftover `/tmp` credential or state files (state, tokens, admin token, owner password, JWKS). Fails the M5 closure if any EP-007 resource survives. Registered in the EP-007 M5 Decision Log.
