/**
 * EP-035 M3 integration test harness: real ephemeral PostgreSQL 18.4 and
 * NATS 2.14.3 containers (digest-pinned per COMPONENT_REGISTRY.yaml).
 *
 * Readiness is proven by connecting through the PUBLISHED HOST PORT,
 * never pg_isready inside the container (EP-001 M5 flake fix: docker's
 * host-port publish can lag the server).
 */
