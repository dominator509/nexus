//! EP-030 OPNsense observability (M2).
//!
//! Bounded, poison-safe observability for the OPNsense connector:
...
//!
//! Telemetry NEVER contains API keys, API secrets, firewall
//! credentials, prompts, or private content (SECURITY.md; SPEC-013
//! security and privacy). Safe telemetry: tenant/device fingerprints,
//! operation, state, error class, correlation id.

use std::collections::{BTreeMap, VecDeque};
use std::sync::atomic::{AtomicU64, Ordering};

/// One audited OPNsense operation (bounded, redacted).
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize)]
pub struct SentinelAuditEntry {
    /// Canonical correlation id for the operation.
    pub correlation: String,
    /// Canonical operation name (e.g. "PROPOSE_CONTAINMENT",
    /// "APPLY_CONTAINMENT", "VERIFY_CONTAINMENT", "REVOKE_CONTAINMENT",
    /// "READ_TELEMETRY").
    ...
    pub detail: String,
    /// Structured SAFE fields: device fingerprint, action kind, error
    /// class. Never raw content or credentials.
    pub fields: BTreeMap<String, String>,
}

/// Bounded redacted audit ring + counters for the OPNsense adapter.
///
...
mod tests {
    ...
    fn ep030_unit_observability_redacts_secrets_at_insert() {
        let mut obs = SentinelObservability::new(256, vec!["sekret-key".to_string()]);
        obs.record(SentinelAuditEntry {
            correlation: "sentinel-1-0".into(),
            operation: "APPLY_CONTAINMENT".into(),
            outcome: "ok".into(),
            detail: "applied with sekret-key".into(),
            fields: BTreeMap::from([("auth".into(), "sekret-key".into())]),
        });
        let audit = obs.audit();
        assert_eq!(audit.len(), 1);
        assert!(!audit[0].detail.contains("sekret-key"));
        assert!(audit[0].detail.contains("***"));
        assert!(!audit[0].fields["auth"].contains("sekret-key"));
        assert_eq!(obs.counter("op:APPLY_CONTAINMENT"), 1);
