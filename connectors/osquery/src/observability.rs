//! EP-031 osquery connector observability (M5).
//!
...
//! (SECURITY.md, SPEC-005). Enrollment secrets and node keys are
//! never recorded.
...
pub struct SentinelAuditEntry {
    ...
    /// Correlation reference (sentinel-<nanos>).
...
/// Redact known sensitive values (poison-safe). osquery enrollment
/// secrets and node keys are never embedded in audit details; any
/// accidental occurrence of a secret-like token is replaced.
fn redact(detail: &str) -> String {
    let mut out = detail.to_string();
    // Redact anything that looks like a bearer/JWT token or long key.
    let mut i = 0;
    while i + 20 < out.len() {
...
mod tests {
    ...
    #[test]
    fn ep031_unit_osquery_observability_redacts_secret_like_tokens() {
        let detail = "node_key abc123def456ghi789jkl";
    ...
    #[test]
    fn ep031_unit_osquery_observability_ring_is_bounded() {
    ...
    #[test]
    fn ep031_unit_osquery_observability_entries_carry_correlation() {
        let obs = SentinelObservability::new();
