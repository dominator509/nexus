#!/usr/bin/env python3
"""EP-024 M3 fixture bootstrap for the REAL pinned Home Assistant provider.

Owns one real HA container (nexus-ep024-ha) for the appliance
integration suite. Authentication and the REST surface REUSE the
EP-020-certified boundary (infra/home-assistant + nexus-home-assistant
transport): the CLI auth `add` verb, the post-restart OAuth login flow
(login_flow -> authorization_code -> access_token), and the documented
HA REST API. No credentials are checked in; a fresh token is minted
per run and handed to the test process through an environment file.

Classification:
  - Home Assistant server/API: REAL_EXTERNAL_DEPENDENCY
  - input_boolean.nexus_app_switch / fan.nexus_app_fan:
    CONTROLLED_TEST_FIXTURE (real entities, real service calls; not
    physical hardware)
...
CONFIG_DIR = ROOT / "connectors/appliances/fixture/config"
ENV_FILE = pathlib.Path("/tmp/ep024-ha.env")
...
EXPECTED_ENTITIES = (
    "input_boolean.nexus_app_switch",
    "fan.nexus_app_fan",
)
...
def _wait_preauth_boot(timeout: int = 300) -> None:
    ...
    The auth-restart must not interrupt this boot or the fixture
    entities are never registered. Pre-auth /api/states is
    unreachable (401), so readiness is observed from the real entity
    registry .storage file (container-side signal).
    """
    deadline = time.time() + timeout
    while time.time() < deadline:
        reg = _docker(
...
def _assert_config_mount() -> None:
    ...
    assert str(CONFIG_DIR) in mounts, (
        f"config mount is not the EP-024 fixture config: {mounts}"
    )
...
def _bootstrap_auth() -> str:
    ...
    # CLI users load from the auth store at boot; restart, then
    ...
    flow = post_json(
        "/auth/login_flow",
        {
            ...
            "redirect_uri": "http://localhost:8124/",
        },
    )
    step = post_json(
...
def start() -> None:
    inspect = _docker(
        ["image", "inspect", f"{IMAGE}@{DIGEST}"], check=False
    )
    ...
    print(f"EP-024 HA fixture ready: {BASE} container={NAME}")
...
def teardown() -> None:
    ...
    try:
        ...
        pass
    print("EP-024 HA fixture torn down")
