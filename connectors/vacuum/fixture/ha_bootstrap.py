#!/usr/bin/env python3
"""EP-024 M5 vacuum fixture bootstrap for the REAL pinned Home Assistant provider.

Owns one real HA container (nexus-ep024-vac) for the vacuum
live-fire/forced-failure integration suite. Authentication and the REST
surface REUSE the EP-020-certified boundary (same flow as EP-020 M3 and
EP-024 M3/M4 fixtures): the CLI auth `add` verb, the post-restart OAuth
login flow (login_flow -> authorization_code -> access_token), and the
...
Classification:
  ...
  - vacuum.nexus_vacuum_a / vacuum.nexus_vacuum_b: CONTROLLED_TEST_FIXTURE
    (real template vacuum entities, real service calls; NOT physical
    robot vacuums)
...
CONFIG_DIR = ROOT / "connectors/vacuum/fixture/config"
ENV_FILE = pathlib.Path("/tmp/ep024-vac.env")

IMAGE = "ghcr.io/home-assistant/home-assistant:stable"
DIGEST = "sha256:56690a89c79a0de98035e1719f8324a92d5859c1192ff45adb0230ea81cb42a5"
NAME = "nexus-ep024-vac"
BASE = "http://127.0.0.1:8126"

EXPECTED_ENTITIES = (
    "vacuum.nexus_vacuum_a",
    "vacuum.nexus_vacuum_b",
)
...
def _assert_config_mount() -> None:
    ...
    assert str(CONFIG_DIR) in mounts, (
        f"config mount is not the EP-024 vacuum fixture config: {mounts}"
    )
...
def _bootstrap_auth() -> str:
    ...
    flow = post_json(
        "/auth/login_flow",
        {
            "client_id": "http://localhost:8126/",
            "handler": ["homeassistant", None],
            "redirect_uri": "http://localhost:8126/",
        },
    )
    step = post_json(
...
def start() -> None:
    ...
    print(f"EP-024 vacuum fixture ready: {BASE} container={NAME}")
...
def teardown() -> None:
    ...
    try:
        ...
        pass
    print("EP-024 vacuum fixture torn down")
