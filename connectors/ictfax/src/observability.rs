//! EP-027 ICTFax observability (M2).
//!
//! Bounded, poison-safe observability for the ICTFax connector:
//!   - a bounded redacted audit ring (latest operations; session
//!     tokens, passwords, and raw document content never stored -
//!     redaction on insert);
//!   - typed counters (operations by outcome/code);
//!   - incident correlation ids (canonical `fax-<nanos>-<seq>`)
//!     preserved across error paths.
//!
//! Telemetry NEVER contains session tokens, passwords, raw document
//! bodies, or private content (SECURITY.md; SPEC-014 privacy). Safe
//! telemetry: transmission/account fingerprints, carrier job id,
//! canonical state, failure class, correlation id.
...
/// Bounded redacted audit ring + counters for the ICTFax adapter.
///
