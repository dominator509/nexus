//! EP-037 M5 S3-compatible transport.
//!
//! REAL transport over plain HTTP/1.1 on std::net TcpStream with a
//! hand-written AWS Signature V4 signer (the same discipline proven
//! against MinIO in M3, SeaweedFS in M4, and RFC 4231). Every request
...
//! Validation/Authorization/RateLimit/Unavailable/ExternalProvider per
//! code.
//!
//! Addressing is path-style ONLY. Virtual-hosted-style (bucket in the
//! hostname) is NOT implemented or asserted: MinIO and the SeaweedFS
//! S3 gateway serve path-style, and AWS S3 is not certified in this
//! milestone. Region/service are explicit configuration, never guessed
//! from the endpoint hostname (SPEC-024: assuming S3 implementations
//! are identical is a non-goal).
//!
...
/// AWS Signature V4 signer (path-style S3, minimal surface). Region and
/// service are explicit configuration (never guessed from the endpoint).
#[derive(Clone)]
pub struct SigV4 {
    access_key: String,
    secret_key: String,
...
impl S3Client {
    pub fn connect(
        ...
        Self {
            ...
            signer: SigV4::new(access_key, secret_key, region),
            connect_timeout,
            read_timeout,
        }
    }
    ...
    /// PUT bucket (location region; empty body canonical). 200 or
...
mod tests {
    ...
    fn sigv4_civil_date_known_value() {
        ...
        let signer = SigV4::new("k", "s", "us-east-1");
        assert_eq!(signer.date_stamp(1787356800), "20260822");
        assert_eq!(signer.amz_date(1787356800), "20260822T000000Z");
    }
