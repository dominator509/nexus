//! EP-026 Microsoft Graph observability (M3).
//!
//! Bounded, poison-safe observability for the Graph connector:
...
/// Bounded redacted audit ring + counters for the Graph adapter.
#[derive(Debug)]
...
mod tests {
    ...
    #[test]
    fn ep026_unit_graph_observability_redacts_secrets_at_insert() {
    ...
    #[test]
    fn ep026_unit_graph_observability_redacts_body_canary_at_insert() {
        // Directive J: full private body content is never retained;
        // a body-shaped canary secret is redacted at insert.
        let body_canary = "EP026BODY_CANARY_9b";
        let mut obs = MailObservability::new(16, vec![body_canary.into()]);
        obs.record(
            "mail-1-2",
            "SEND",
            "ok",
            format!("content {body_canary} sent"),
            BTreeMap::new(),
        );
        let entry = obs.recent().pop().expect("entry");
    ...
    #[test]
    fn ep026_unit_graph_observability_bounded_ring() {
    ...
    #[test]
    fn ep026_unit_graph_observability_counters() {
