/**
 * EP-035 M1 contract validation primitives.
 *
 ...
 * without passing through these checks, so setup can never fabricate
 * vocabulary or authority from unvalidated input. Deserialization
 * (fromJson) and constructor paths enforce equivalent validation.
 */
...
export function assertStringArray(
  ...
  if (value.length > max) {
    throw new Spec006Error(
      ErrorCode.Validation,
      `${what} must contain at most ${max} entries`,
    );
  }
