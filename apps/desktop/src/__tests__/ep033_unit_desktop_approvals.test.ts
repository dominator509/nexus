...
describe("ep033_unit_desktop_approval_flow", () => {
  ...
  it("denies when the requester approves their own action", () => {
    ...
    try {
      flow.apply(
        ApprovalAction.record(uuid(30), "APPROVE", uuid(32), 1),
        1_700_000_000,
      );
      expect.unreachable();
    } catch (error) {
  ...
  it("refuses actions after a terminal state", () => {
    ...
    expect(() =>
      flow.apply(
        ApprovalAction.record(uuid(30), "DENY", uuid(41), 2),
        1_700_000_001,
      ),
    ).toThrowError(Spec006Error);
  });
  ...
  it("rejects actions targeting a different approval card", () => {
    ...
    expect(() =>
      flow.apply(
        ApprovalAction.record(uuid(99), "APPROVE", uuid(40), 1),
        1_700_000_000,
      ),
    ).toThrowError(Spec006Error);
  });
